...
3. **Принимает решение** — LOCAL (локально, бесплатно), CLOUD (облако, платно) или HYBRID (локальная генерация → heuristic → если уверен → LOCAL, иначе CLOUD)
4. Если запрос многоступенчатый — **декомпозирует** на подзадачи (CASCADE) и каждую маршрутизирует отдельно
5. **Self-assessment** — после генерации анализирует ответ (длина, структура, uncertainty-фразы), а не отправляет повторный LLM-запрос
...
| **Self-Assessment** | Анализ ответа эвристиками (длина, структура, uncertainty-фразы). Без второго LLM-запроса — экономия времени и токенов |
| **Domain Thresholds** | Разные пороги для кода, анализа, чата — код чаще локально, анализ — в облако |
| **Cascade Decomposition** | Сложные многоступенчатые запросы разбиваются на подзадачи. Если все подзадачи одного типа — консолидируются в один вызов |
...
> **⚠️ Важно:** Cascade-режим (декомпозиция многоступенчатых запросов) и Hybrid-режим требуют локальной модели со скоростью генерации **от 25 токенов/с**. При более медленных моделях возможны таймауты (дефолтный timeout — 300с).
...
- Запросы с "и", "затем", "сначала", "после этого", "потом" → декомпозиция на подзадачи
- Одиночные предлоги ("with", "также") не вызывают декомпозицию
...
```
Запрос → LM Studio (ваша модель) → Ответ + Heuristic Confidence
```

**Heuristic Confidence** — анализ ответа без второго LLM-запроса:
- Учитывается: длина ответа, наличие кода, структурированность (заголовки, списки), uncertainty-фразы ("не знаю", "don't know")
- Если confidence ≥ порог домена → **LOCAL**
- Если confidence < порог → fallback в **CLOUD**

Никаких дополнительных токенов на self-assessment.

### 5. Обработка в гибридном режиме

```
Запрос → Локальная модель (полная генерация) → Heuristic Confidence
           ↓ confident                                              ↓ low
        LOCAL (0 токенов облака)                  CLOUD (fallback)
```

HYBRID больше не делает "локальную выжимку + обязательно облако". Теперь он пытается сгенерировать ответ локально, и только если heuristic даёт низкую уверенность — уходит в облако. Это позволяет HYBRID-режиму тоже экономить токены.
...
Декомпозиция:
  ...
  2. "Создай отчет" → HYBRID
  3. "Добавь графики" → HYBRID
```

Каждая подзадача маршрутизируется отдельно: все типы (local/hybrid/cloud) проходят через heuristic и имеют шанс остаться локально. Если все подзадачи требуют облака — консолидируются в один вызов вместо N.
...
local_model:
  ...
  timeout_seconds: 300  # увеличено для медленных моделей
```

### Квоты и таймауты

```yaml
max_tokens: null          # null = модель решает сама
local_model:
  timeout_seconds: 300
```
...
- `max_tokens` (опц.) — макс. токенов в ответе (`null` = модель решает)
...
**Особенности:**
- Поддерживает `stream: True` — SSE-стрим первого токена
- Поддерживает `max_tokens` из запроса (не хардкодится)
